...
public:
	...
	static const unsigned int IRWXG = ((IREAD | IEXEC) >> 3);			    /* 文件主同组用户对文件的读、执行权限 */
	static const unsigned int IRWXO = ((IREAD) >> 6);			    /* 其他用户对文件的读权限 */
...
public:
    unsigned int i_flag;	// 状态的标志位，定义见enum INodeFlag, 记录iNode是否被访问或修改
...
public:
    ...
    void ReadI();//读其实是将文件内容写入到用户空间的内存中

	/* 根据Inode对象中的物理磁盘块索引表，将数据写入文件 */
    void WriteI();//写其实是将用户空间的内存写回到磁盘
