...
void FileSystem::FormatDevice() {
	FormatSuperBlock();
	deviceDriver->Construct(); //创建磁盘镜像文件
...
/* 在存储设备上分配空闲磁盘块,并返回该磁盘块对应的缓存控制块,方便后续写入 */
Buffer* FileSystem::Alloc() {
	...
	/* 若获取磁盘块编号为零，则表示已分配尽所有的空闲磁盘块。成组链接法中的最后一组*/
	if (blkno <= 0) {
		superBlock->s_nfree = 0;
	...
	*/
	if (superBlock->s_nfree <= 0) {
		...
		int* p = (int*)pBuffer->addr;  //强制类型转换, 改成int类型的数组方便使用
		superBlock->s_nfree = *p++;
		memcpy(superBlock->s_free, p, sizeof(superBlock->s_free)); //把组长块从缓存加载到栈中
		this->bufferManager->Brelse(pBuffer);//释放缓存
	}
	pBuffer = this->bufferManager->GetBlk(blkno);  //把磁盘中下一组的组长块拿来做存储,返回缓存控制块
	if (pBuffer) {
...
void FileSystem::Free(int blkno) {
	...
	if (superBlock->s_nfree >= SuperBlock::MAX_NFREE) {//如果栈满了,被释放的磁盘块将作为组长块存储在磁盘中
	...
	superBlock->s_free[superBlock->s_nfree++] = blkno;//指向待释放的磁盘块
	superBlock->s_fmod = 1;
}