...
class SuperBlock {
public:
	const static int MAX_NFREE = 100;  //栈中最大空闲盘块数量
	const static int MAX_NINODE = 100; //栈中最大iNode数量

public:
	int	s_isize;		        // 外存INode区占用的盘块数
	int	s_fsize;		        // 盘块总数

	int	s_nfree;		        // 直接管理的空闲盘块数量,可以当做栈的指针来用
	int	s_free[MAX_NFREE];	    // 直接管理的空闲盘块索引表, 以栈的形式进行管理
...
public:
	...
	// 文件系统user1目录外存INode编号				etc99	password98
	static const int USER1_INODE_NO = 97;
	// 文件系统user2目录外存INode编号
	static const int USER2_INODE_NO = 96;
	// 文件系统home目录外存INode编号
	static const int HOME_INODE_NO = 100;
...
public:
	...
	/* 格式化SuperBlock */
	void FormatSuperBlock();  //修改内存中的超级块数据
