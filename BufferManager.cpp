...
void BufferManager::InitList() {//初始化双向循环链表, 建立缓存控制块前后指向关系
    for (int i = 0; i < NBUF; ++i) {
        if (i) {
...
/* 申请一块缓存，从缓存队列中取出，用于读写设备上的块blkno.只是在缓冲控制块中标记,并未真正读入*/
Buffer* BufferManager::GetBlk(int blkno) {
    Buffer* pb;
    if (map.find(blkno) != map.end()) {//如果原来内存就有存储第blkno号的内存块,就返回它
        pb = map[blkno];
        DetachNode(pb);
        return pb;
    }
    ...
    DetachNode(pb); //没有就取出第一个
	map.erase(pb->blkno);
    if (pb->flags & Buffer::B_DELWRI) {  //检查原来的缓存上的内容是否需要写回
